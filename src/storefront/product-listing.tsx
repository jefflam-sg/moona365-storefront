...
export function ProductListingPage({ title, description, eyebrow, basePath, query, listing, filters, breadcrumbs = [], childCategories = [] }: { title: string; description?: string; eyebrow: string; basePath: string; query: Query; listing: PublicProductListing; filters: Filters; breadcrumbs?: Array<{ label: string; href?: string }>; childCategories?: PublicCategory[] }) {
  ...
  return <>
    ...
    <div className="sf-plp-layout" data-no-filters={systemFilters.length === 0}>
      {systemFilters.length > 0 && <aside className="sf-filter-sidebar"><div className="sf-filter-title"><h2>Filter By</h2>{activeCount > 0 && <Link href={hrefWith(basePath, query, { minPrice: null, maxPrice: null, inStock: null, page: null })}>Clear all</Link>}</div><FilterForm basePath={basePath} query={query} filters={systemFilters} /></aside>}
