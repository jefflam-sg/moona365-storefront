import type { HomepageCatalogue, NavigationDestination, NavigationNode, PublicCategory, WebsiteCustomPage, WebsiteNavigation } from "./contracts";
...
function destination(destination: NavigationDestination, catalogue: HomepageCatalogue, pages: WebsiteCustomPage[]): { href: string | null; label: string | null; category?: PublicCategory } | null {
  ...
  if (destination.type === "CUSTOM_PAGE") { const page = pages.find((item) => item.id === destination.pageId && item.status === "ACTIVE"); return page ? { href: `/pages/${page.slug}`, label: page.title } : null; }
...
export function resolveNavigation(kind: "main" | "footer", navigation: WebsiteNavigation | undefined, catalogue: HomepageCatalogue, pages: WebsiteCustomPage[] = []): ResolvedNavigationNode[] {
  ...
  const walk = (node: NavigationNode): ResolvedNavigationNode | null => {
    const target = destination(node.destination, catalogue, pages);
    ...
    const promoTarget = node.megaMenu?.promo ? destination(node.megaMenu.promo.destination, catalogue, pages) : null;
