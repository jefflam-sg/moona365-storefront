...
export default async function CollectionPage({ params, searchParams }: { params: Promise<{ slug: string }>; searchParams: Promise<ListingQuery> }) {
  ...
  const listing = await loadStorefrontListing(result.resolvedHost, { source: "collection", collectionId: collection.id, limit: 12, ...query }); if (!listing) notFound();
  return <StorefrontPageShell snapshot={result.snapshot} catalogue={catalogue}><ProductListingPage title={collection.name} description={collection.description} eyebrow="COLLECTION" basePath={`/collections/${collection.slug}`} query={query} listing={listing} breadcrumbs={[{ label: "Collections", href: "/collections" }, { label: collection.name }]} /></StorefrontPageShell>;
}
