...
export default async function CategoryPage({ params, searchParams }: { params: Promise<{ slug: string[] }>; searchParams: Promise<ListingQuery> }) {
  ...
  const listing = await loadStorefrontListing(result.resolvedHost, { source: "category", categoryId: category.id, includeSubcategories: category.hasChildren, limit: 12, ...query }); if (!listing) notFound();
  ...
  return <StorefrontPageShell snapshot={result.snapshot} catalogue={catalogue}><ProductListingPage title={category.name} eyebrow="CATEGORY" basePath={`/categories/${canonicalSlug}`} query={query} listing={listing} breadcrumbs={breadcrumbs} childCategories={children} /></StorefrontPageShell>;
}
