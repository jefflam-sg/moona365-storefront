...
export default async function ProductPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ variant?: string | string[] }> }) {
  ...
  const initialVariantId = typeof requestedVariant === "string" && product.variants.some((variant) => variant.id === requestedVariant) ? requestedVariant : undefined;
  if (isInternalId) redirect(`/products/${product.slug}${initialVariantId ? `?variant=${encodeURIComponent(initialVariantId)}` : ""}`);
  ...
  return <StorefrontPageShell snapshot={result.snapshot} catalogue={catalogue ?? undefined}><ProductDetail product={product} initialVariantId={initialVariantId} recommendations={recommendations} bottomSections={productSections} catalogue={catalogue ?? undefined} /></StorefrontPageShell>;
}
