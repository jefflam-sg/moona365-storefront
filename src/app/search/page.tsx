...
export default async function SearchPage({ searchParams }: { searchParams: Promise<ListingQuery> }) {
  ...
  const [listing, catalogue, filters] = await Promise.all([loadStorefrontListing(result.resolvedHost, { source: "search", search: query.q, limit: 12, ...query }), loadNavigationCatalogue(result.resolvedHost, result.snapshot), loadStorefrontFilterConfiguration(result.resolvedHost)]);
  if (!listing || !catalogue || !filters) notFound();
  return <StorefrontPageShell snapshot={result.snapshot} catalogue={catalogue}><ProductListingPage title={`Results for “${query.q}”`} eyebrow="SEARCH" basePath="/search" query={query} listing={listing} filters={filters.filterSet.items} breadcrumbs={[{ label: "Search" }]} /></StorefrontPageShell>;
}
